...
import classes from "./FormPartner.module.scss";
import TextTitle from '@/ui/components/TextTitle';

export default function FormPartner() {
  const router = useRouter();
  const [formData, setFormData] = useState({ name: "", position: "", company: "", phone: "", email: "" });
  ...
  const validate = () => {
    ...
    if (!formData.position.trim()) newErrors.position = "Вкажіть посаду";
    if (!formData.company.trim()) newErrors.company = "Вкажіть компанію";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setFormData({ name: "", position: "", company: "", phone: "", email: "" });
        setErrors({});
  ...
  return (
      <div className={classes.formWrapper}>
        <form onSubmit={handleSubmit} className={classes.form}>
          <div className={classes.formMainWrap}>
            ...
            <TextTitle className={classes.wrap}>
              <input className={`${classes.input} ${errors.position ? classes.inputError : ""}`} type="text" name="position" placeholder="Посада" value={formData.position} onChange={handleChange} />
            ...
            <TextTitle className={classes.wrap}>
              <input className={`${classes.input} ${errors.company ? classes.inputError : ""}`} type="text" name="company" placeholder="Компанія" value={formData.company} onChange={handleChange} />
              {errors.company && <p className={classes.error}>{errors.company}</p>}
            </TextTitle>
            <TextTitle className={classes.wrap}>
          ...
          <button type="submit" className={classes.btn}>Надіслати</button>
        </form>
      </div>
  );
}